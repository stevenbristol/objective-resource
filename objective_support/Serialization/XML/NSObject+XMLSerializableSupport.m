...
+ (id)fromXMLData:(NSData *)data {
	...
	bool hasData = delegate.parsedObject;
    [parser release];
	if (!parsed || !hasData){
		return [NSError errorWithDomain:@"NetworkError" code:(NSInteger)0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected data returned from server" forKey:NSLocalizedDescriptionKey]];
	}
